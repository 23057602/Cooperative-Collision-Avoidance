...
from spline_works.spline_works import Spline, Spline_make
...
class Stanley_controller(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("The Stanley controller has started...")
        ...
        self.declare_parameter('steering_limit', pi/3, ParameterDescriptor(description='Maximum steering angle in radians.'))
        ...
        self.declare_parameter('top_speed', 1.0, ParameterDescriptor(description='In/Along-track controller maximum catch up speed.'))
        self.declare_parameter('crawl_speed', 0.01, ParameterDescriptor(description='In/Along-track controller minimum lag speed.'))
        ...
        self.turn_clip = self.get_parameter('steering_limit').get_parameter_value().double_value
        ...
        self.ctrl_v_hi = self.get_parameter('top_speed').get_parameter_value().double_value
        self.ctrl_v_lo = self.get_parameter('crawl_speed').get_parameter_value().double_value
        self.server = ActionServer(self, Drive,"Controller_Server/follow_reference",goal_callback=self.receiveGoal,execute_callback=self.actionMonitor, cancel_callback=self.cancelGoal, callback_group=ReentrantCallbackGroup())
    ...
    def receiveGoal(self, Goal: ServerGoalHandle):
        ...
        self.g_ref_list = [
            [
                Spline(
                    Spline_make.hermite_mtx, 
                    Spline_make.deserialize(
                        Goal.path_reference.seq[i].path.mtx_data,
                        Goal.path_reference.seq[i].path.ROWS,
                        Goal.path_reference.seq[i].path.cols
                        )
                    ), 
                Goal.path_reference.seq[i].speed, 
                Goal.path_reference.seq[i].start_time, 
                Goal.path_reference.seq[i].stop_time
            ] for i in range(len(Goal.path_reference.seq))][::-1] # create global plan stack
        ...
        self.l_ref_list = [
            [
                Spline(
                    Spline_make.hermite_mtx, 
                    Spline_make.deserialize(
                        plan.ref_stack.seq[i].path.mtx_data,
                        plan.ref_stack.seq[i].path.ROWS,
                        plan.ref_stack.seq[i].path.cols
                        )
                    ),
                plan.ref_stack.seq[i].speed,
                plan.ref_stack.seq[i].start_time,
                plan.ref_stack.seq[i].stop_time
            ] for i in range(len(plan.ref_stack.seq))][::-1] # create local plan stack
    ...
    def stanley(self):
        ...
        #compute control law variables
        first = 0
        last = len(self.g_ref_list)
        while(first+1 < last): # binary search for current time path reference
            if(self.get_clock().now().nanoseconds >= rclpy.time.Time.from_msg(self.g_ref_list[int((first + last)/2)][2]).nanoseconds):
                first = (first + last)/2
            else:
                last = (first + last)/2
        c_ref_ptr = int((first + last)/2) # current time reference
        ...
        cross_len = (2*(c_ref_ptr > self.g_ref_ptr[0]) - 1) * sum([r[0].len for r in self.g_ref_list[min(c_ref_ptr,self.g_ref_ptr[0]):max(c_ref_ptr,self.g_ref_ptr[0])]]) # add up spline lengths between current and time reference
        ...
        if(t >= 1.0):#check if reference complete
            ...
            vhl_cmd.linear.x = v
            vhl_cmd.angular.z = v*tan(ctrl_Lw)/self.wheel_base #steering to angular rate conversion
        self.control_command_stream.publish(vhl_cmd)
        
        #record data
        self.prog = curr_len/ref_list[ref_ptr[0]][0].len
        state = DriveState()
        ...
        state.path.mtx_data = Spline_make.serialize(ref_list[ref_ptr[0]][0].M.tolist())
