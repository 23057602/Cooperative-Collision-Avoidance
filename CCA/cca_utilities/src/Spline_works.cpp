...
Eigen::Matrix4Xd Spline_make::fromPoints(Eigen::MatrixXd point_column, Eigen::Matrix4d char_mtx)
{
    ...
    char_mtx.computeInverseWithCheck(char_mtx_inv, invertable);
    if(!invertable){throw std::invalid_argument( "char_mtx is not invertable." );}

    Eigen::Matrix4d T_t_T_inv;
    Eigen::Matrix4d(T.transpose() * T).computeInverseWithCheck(T_t_T_inv, invertable);
    if(!invertable){throw std::invalid_argument( "Data rank is less than 4." );}
